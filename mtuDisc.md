...
```
- 另。当找到了正确的size，也仍旧ping不通，也可能对端已经禁ping，但是报错与size过大是不同的。
...
- 若预测MTU为1500，则`packetsize`设置为1472可通过，预留28差值，1473失败。因为ip头长度为20byte，icmp头长度为8byte，MTU = iphl + icmphl + icmpdatal。
...
- 结论：tun实际有效MTU长度为 1390 + 8 = 1398（根据icmp估算，其他协议结论相同，只不过协议头长度不同，数据段长度不同）
